...
from schemas.schema import formulaire_profil_utilisateur, \
    formulaire_demande_inspection, formulaire_supprimer_inspection
...
INFRACTION_URL = "https://data.montreal.ca/dataset" \
                 "/05a9e718-6810-4e73-8bb9-5955efeb91a0/resource" \
                 "/7f939a08-be8a-45e1-b208-d8744dca8fc6" \
...
# Fonction pour extraire les données de la ville de Montréal
# et les mettre à jour dans la base de données
def update_database():
    url = INFRACTION_URL
    ...
    with sqlite3.connect('db/db.db') as conn:
        ...
        for contrevenant in contrevenants:
            # Vérifier si la ligne existe déjà dans la base de
            # données avant de l'insérer
            cursor.execute("SELECT * FROM contrevenants "
                           "WHERE id_poursuite = ?", (contrevenant[0],))
...
# Sert à lister les changements lorsqu'il y a des mises
# à jour au fichier csv de la ville
def creer_liste_nouveaux_changements():
    liste_des_nouveaux_id = update_database()

    for new_id in liste_des_nouveaux_id:
        new_data = get_db().get_list_new_contrevenants(new_id) \
            .append()
...
# Configuration du BackgroundScheduler pour exécuter la
# fonction update_database() chaque jour à minuit
...
def recherche_etablissement():
    ...
    filter_contrevenants = \
        _filter_contrevenants_etablissement(contrevenants, query)
    return render_template('resultat.html',
                           contrevenants=filter_contrevenants)
...
def recherche_proprietaire():
    ...
    contrevenants = get_db().get_all_contrevenants()
    filter_contrevenants = \
        _filter_contrevenants_proprietaire(contrevenants, query)
    return render_template('resultat.html',
                           contrevenants=filter_contrevenants)
...
# Sert au service REST permettant d'obtenir la liste des contrevenants
# ayant commis une infraction entre deux dates
...
def get_contrevenant():
    ...
    try:
        ...
        response = {'error': 'Format de date invalide. '
                             'Utilisez le format ISO 8601 (AAAA-MM-JJ).'}
        return jsonify(response), 400

    filter_contrevenants = [c for c in contrevenants if
                            start_date <= datetime
                            .strptime(c['date_infraction'], '%Y-%m-%d') <=
                            end_date]
...
def create_user():
    ...
    try:
        ...
        return {"status": "error",
                "message": "Validation failed", "errors": str(e)}, 422
    ...
    if nom_user == "" or prenom_user == "" or password == "" \
...
def get_etablissements_xml():
    try:
        ...
        return render_template('etablissement-par-contraventions.xml',
                               contrevenants=contrevenants), 200
...
# demande d'inspection à la ville.
...
def create_inspection_request():
    ...
    try:
        ...
        return {"status": "error", "message": "Validation échouée",
                "errors": str(e)}, 422
    ...
    if etablissement == "" or nom_user == "" or prenom_user == "" \
            or adresse == "" or ville == "" or date_visite == "" \
            or probleme == "":
        return jsonify({'error': 'svp remplir tous les champs demandés'}), 400
    else:
...
@app.route("/api/delete-inspection", methods=["DELETE"])
def delete_inspection_request():
    json_data = request.get_json()

    try:
        validate(instance=json_data,
                 schema=formulaire_supprimer_inspection)
    except Exception as e:
        return {"status": "error", "message": "Validation échouée",
                "errors": str(e)}, 422
